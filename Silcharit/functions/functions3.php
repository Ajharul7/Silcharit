	...
	if(isset($_POST['sub3'])){
		...
		if(strlen($content) > 250){
			...
			if(strlen($upload_image) >= 1 && strlen($content) >= 1){
				...
				$insert = "insert into posts3(user_id, post_content, upload_image, post_date) values('$user_id', '$content', '$upload_image.$random_number', NOW())";
				...
				if($run){
					...
					$update = "update users set posts3='yes' where user_id='$user_id'";
				...
				if($upload_image=='' && $content == ''){
					...
					if($content==''){
						move_uploaded_file($image_tmp, "imagepost/$upload_image.$random_number");
						$insert = "insert into posts3 (user_id,post_content,upload_image,post_date) values ('$user_id','No','$upload_image.$random_number',NOW())";
						$run = mysqli_query($con, $insert);
						...
						$insert = "insert into posts3 (user_id, post_content, post_date) values('$user_id', '$content', NOW())";
						$run = mysqli_query($con, $insert);
...
function get_posts3(){
	...
	$get_posts3 = "select * from posts3 ORDER by 1 DESC LIMIT $start_from, $per_page";

	$run_posts = mysqli_query($con, $get_posts3);
		...
		if($content=="No" && strlen($upload_image) >= 1){
			...
			<div class='move'>
				<div id='posts' class='col-sm-6'>
					...
					<a href='single3.php?post_id=$post_id' style='float:right;'><button class='btn btn-info'>Comment</button></a><br>
				</div>
				<div class='col-sm-3'>
				</div></div>
			</div><br><br>
	...
	// include("pagination.php");
		...
		function single_post3() 
		{
			if(isset($_GET['post_id'])) {
				...
				$get_posts2 = "select * from posts3 where post_id='$get_id'";
				...
				$post = $_GET['post_id'];
				$get_user = "select * from posts3 where post_id='$post'";
		...
		<div class='panel-body'>
				...
				if(isset($_POST['reply'])) {
					...
					echo "<script>window.open('single3.php?post_id=$post_id','_self')</script>";
						}
						else{
							$insert = "insert into comments3
